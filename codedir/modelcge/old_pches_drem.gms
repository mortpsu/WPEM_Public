...
$if not set s_pw $set s_pw 0

* PSM (with productivity impacts)
$if not set s_pf $set s_pf 0
...
* Save level values
$set save_valuelevel 'output\%target%\gdx\%mod%_=level.gdx'

* Save percentage change values
$set save_valuepchg  'output\%target%\gdx\%mod%_=pchg.gdx'

* Save growth rates
$set save_growthrate 'output\%target%\gdx\%mod%_growth.gdx'

* Save model parameters
$set save_parameters 'output\%target%\gdx\%mod%_parameters.csv'

$set save1_parameters 'output\%target%\csv\%mod%_level.csv'
$set save2_parameters 'output\%target%\csv\%mod%_pchg.csv'
$set save3_parameters 'output\%target%\csv\%mod%_growth.csv'
...
* Save growth rates
$set save_growthrate 'output/gdx/%mod%_rate'
...
PARAMETER
        amc_chg(r_a)                  PSM's chg in average marginal cost from shock in region r
        ChangeCali                    PSM percentage change in electricity demand for California
        ChangeRO                      PSM percentage change in electricity demand for ROWECC
        psmtfp_temp(r,g,s,tp,sim)   Temporary PSM's chg in average marginal cost from shock in region r
        psmtfp_fa_temp(r,s,tp,sim)  Temporary PSM's chg for factors in average marginal cost from shock in region r
;

$GDXIN %load_psm_ele%
$LOAD ChangeCali ChangeRO
$GDXIN
...
psmtfp_temp(cal,g,'ele',tlast,sim_psm)  = 1 + amc_chg('CA');
psmtfp_fa_temp(cal,'ele',tlast,sim_psm) = 1 + amc_chg('CA');

psmtfp_temp(rwc,g,'ele',tlast,sim_psm)  = 1 + amc_chg('ROWECC');
psmtfp_fa_temp(rwc,'ele',tlast,sim_psm) = 1 + amc_chg('ROWECC');
...
psmtfp_fa_temp(rwc,nele,tlast,sim_psmfull) = 1 + Sull_impacts(nele,'ROWECC');
        ...
        DISPLAY Sull_impacts, psmtfp_fa_temp, psmtfp_temp;
...
psmtfp_fa_temp(rwc,'ele',tlast,policy) = 1 + amc_chg('ROWECC');
        ...
        DISPLAY psmtfp_fa_temp, psmtfp_temp;
